...
public class InvoicesControllerIntegrationTests : IClassFixture<CustomWebApplicationFactory>, IDisposable
{
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/invoices/current");
    ...
    public async Task PostInvoice_CreatesNewInvoice()
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync("/api/invoices", amount);
        ...
        var savedInvoice = await _dbContext.Invoices
            .FirstOrDefaultAsync(i => i.Year == today.Year && i.Month == today.Month);
    ...
    public async Task UpdateInvoice_UpdatesExistingInvoice()
    {
        ...
        // Act
        var response = await _client.PutAsJsonAsync($"/api/invoices/{year}/{month}", updateModel);
