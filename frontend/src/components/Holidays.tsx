...
const Holidays: React.FC = () => {
  ...
  return (
    <div className="bg-[#161b22] rounded-lg shadow-lg p-3 md:p-4 xl:p-4 3xl:p-6 border border-[#30363d] h-full min-h-[200px] lg:min-h-0">
      <h3 className="text-xl md:text-2xl xl:text-2xl 3xl:text-3xl font-bold text-slate-200 mb-2 md:mb-3 xl:mb-3 3xl:mb-4">
        Helligdage næste 7 dage
      </h3>
      {isLoading ? (
        <div className="flex items-center justify-center h-[calc(100%-3rem)] md:h-[calc(100%-3.5rem)] xl:h-[calc(100%-3.5rem)] 3xl:h-[calc(100%-4rem)]">
          <p className="text-slate-400 text-base md:text-lg xl:text-lg 3xl:text-2xl">Indlæser helligdage...</p>
        </div>
      ) : error ? (
        <div className="flex items-center justify-center h-[calc(100%-3rem)] md:h-[calc(100%-3.5rem)] xl:h-[calc(100%-3.5rem)] 3xl:h-[calc(100%-4rem)]">
            ...
            <div key={dateStr} className="bg-[#0d1117] rounded-lg p-2 md:p-2.5 xl:p-2.5 3xl:p-3 border border-[#30363d] flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto space-y-1.5 md:space-y-2">
                {holidaysOnDate.map((holiday, index) => (
                  <div key={`${holiday.countryCode}-${holiday.name}-${index}`} className="bg-[#161b22] rounded p-2 md:p-2.5 xl:p-2.5 3xl:p-3 border border-[#30363d]">
                    <div className="font-bold text-slate-100 text-sm md:text-base xl:text-base 3xl:text-xl mb-1 md:mb-1.5 xl:mb-1.5 3xl:mb-2">{holiday.countryName}</div>
                    <div className="text-slate-400 text-xs md:text-sm xl:text-sm 3xl:text-base leading-snug" title={holiday.name}>{holiday.name}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex items-center justify-center h-[calc(100%-3rem)] md:h-[calc(100%-3.5rem)] xl:h-[calc(100%-3.5rem)] 3xl:h-[calc(100%-4rem)]">
          <p className="text-slate-400 text-base md:text-lg xl:text-lg 3xl:text-2xl">Ingen helligdage de næste 7 dage.</p>
        </div>
      )}
    </div>
  );
