              ...
              <div className="flex-1 overflow-y-auto space-y-1.5 md:space-y-2 xl:space-y-3 3xl:space-y-4 4k:space-y-6">
                {holidaysOnDate.map((holiday, index) => (
                  <div key={`${holiday.countryCode}-${holiday.name}-${index}`} className="bg-[#161b22] rounded p-2 md:p-3 xl:p-4 3xl:p-6 4k:p-10 border border-[#30363d]">
                    <div className="font-bold text-slate-100 text-base md:text-xl xl:text-2xl 3xl:text-4xl 4k:text-[6rem] mb-1 md:mb-2 xl:mb-3 3xl:mb-4 4k:mb-6">{holiday.countryName}</div>
                    <div className="text-slate-400 text-sm md:text-lg xl:text-xl 3xl:text-3xl 4k:text-[4rem] leading-snug" title={holiday.name}>{holiday.name}</div>
                  </div>
                ))}
              </div>
            </div>
